...
public class FgModelHelper {
    ...
    public static void buildNativeModel(BundleResource filename, ResourcePath opttexturepath, ModelBuildDelegate modeldelegate, int options) {
        ...
        Platform.getInstance().buildNativeModelPlain(mapFilename(filename, true, options), opttexturepath, modeldelegate, options);
    }

    /**
     * 18.10.23: ac->gltf name mapping and policy setting now here.
     */
    public static SceneNode mappedasyncModelLoad(BundleResource resource){
        int options = EngineHelper.LOADER_USEGLTF;
        if (resource.getExtension().equals("ac")) {
            options |= LOADER_APPLYACPOLICY;
        }
        return ModelFactory.asyncModelLoad(FgModelHelper.mapFilename(resource, true, options), options);
    }
