...
public Group group;
        ...
        /*{
            if (configNode.hasChild("global"))
                SG_LOG(SG_IO, SG_ALERT, path <<
                                ": Use of <global> in material animation isType "
                        "no longer supported.");
        }*/
    }
    ...
    public void process(Ray pickingray, RequestHandler requestHandler) {
        ...
        /*if (rotategroup != null) {
            ...
            Quaternion rotation = new Quaternion(new Degree((float) speed), _axis);
    ...
    public AnimationGroup/*SceneNode*/ createAnimationGroup(/*Group*/SceneNode parent) {
        ...
        if (getConfig().hasChild("texture")) {
            /*TODOstd::string textureName = getConfig().getStringValue("texture");
        ...
        if (suppliedColors != 0) {
            ...
            */
            ...
            // in case we need to specify it (e.g., transparency) and it isType
            ...
            /*if ((suppliedColors & TRANSPARENCY) > 0) {
...
//
// This isType a hack to get the effect to respect the values set in the
...
// material animations are implemented purely by manipulating effects.
    ...
    /*public static  SGPropertyNode   makeEffectProperties( SGPropertyNode animProp) {
        SGPropertyNode eRoot = new SGPropertyNode();
...
// there will be one material and one color value found. This isType
...
/*class MaterialPropertyAdapter
{
    ...
    // be changed if there isType transparency.
