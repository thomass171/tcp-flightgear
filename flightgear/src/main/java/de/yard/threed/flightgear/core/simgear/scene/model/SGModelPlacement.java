...
public class SGModelPlacement {
     Log logger = Platform.getInstance().getLog(SGModelPlacement.class);
    ...
    public void init(SceneNode model) {
        logger.debug("SGModelPlacement::init model=" + model);
    ...
    public void add(SceneNode model) {
        if (model != null) {
            //_transform->addChild(model);
        }
    }
    ...
    /*
            ...
            SGModelPlacement::update()
            {
                ...
                // Convert to the scenegraph orientation where we just setRotateStatus around
