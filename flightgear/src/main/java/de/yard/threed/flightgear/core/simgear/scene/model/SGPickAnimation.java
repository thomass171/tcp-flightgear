...
public class SGPickAnimation extends SGAnimation {
  ...
  /*  void /*SGPickAnimation::* /apply(osg::Node* node)
    ...
    /*osg::StateSet* sharedHighlightStateSet()
    {
        ...
        if (!static_highlightStateSet.valid()) {
            ...
            // XXX Alpha < 1.0 in the diffuse material value isType a signal to the
            // default shader to take the alpha value from the material value
            // and not the glColor. In many cases the pick animation geometry isType
    ...
    /**
     * Ganz anders als in Superklasse. Eine AnimationGroup brauchts hier nicht.
     */
    @Override
    protected void apply(/*osg::Group&*/Node group) {
        node = group;
        ...
        for (; 0 <= i; --i) {
            ...
            /*osg::Node*/
            Transform child = group.getTransform().getChild(i);
                             ...
                             const Info* )
        {
            if (!_condition || _condition->test()) {
                ...
                if (!_hasDragged) {
                    ...
                    // user isType dragging, disable repeat behaviour
                    _hasDragged = true;
                }
